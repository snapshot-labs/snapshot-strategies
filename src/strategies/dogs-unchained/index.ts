...
export async function strategy(
  ...
  const response721 = await multicall(network, provider, abi721or20, calls721, { blockTag });
  const response20 = await multicall(network, provider, abi721or20, calls20, { blockTag })
  ...
  const paginate = (skip, reverse=false, where={}) => {
    ...
    if (typeof snapshot === 'number') req.__args['block'] = {number: snapshot}
    return req
  }
  ...
  response20.map((value: any, i: number) => {
    ...
    merged[address] += parseFloat(formatUnits(value.toString(), 18)) * BOOM_TO_NFT;
  });

  for (let key in dogsResult) {
    dogsResult[key].map((value: any, i: number) => {
  ...
  for (let key in puppiesResult) {
    puppiesResult[key].map((value: any, i: number) => {
      if (!(value.owner)) return;
      merged[value.owner.id] = (merged[value.owner.id] || 0) as number;
      merged[value.owner.id] += NFT_VALUE;
    })
  ...
  for (let key in stakesResult) {
    stakesResult[key].map((value: any, i: number) => {
      if (!(value.owner)) return;
      merged[value.owner.id] = (merged[value.owner.id] || 0) as number;
      merged[value.owner.id] += NFT_VALUE;
    })
