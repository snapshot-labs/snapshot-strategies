...
const HOPR_STAKING_SUBGRAPH_URL =
  'https://api.thegraph.com/subgraphs/name/hoprnet/hopr-staking-program';
...
async function stakingSubgraphQuery(
  ...
  const entries = data.accounts.map((d) => [
    d.id,
    BigNumber.from(d.actualStake).add(
      BigNumber.from(d.virtualStake).add(BigNumber.from(d.unclaimedRewards))
    )
  ]);
