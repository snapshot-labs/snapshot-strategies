...
export async function strategy(
    ...
    Object.entries(balance).map(async ([address, balance]) => {
        ...
        for (let i=0; i<num; i++){
            ...
            nftIdCall.call(path, options.address, 'tokenOfOwnerByIndex', [address, i])
            nftIdCall.call(stakedPath, options.address, 'stakedNft', [address])
        } 
    })
    ...
    Object.entries(points).map(([path, point])=>{
        ...
        const tmp = Object.keys(ans).find((t)=>{return t===owner})
