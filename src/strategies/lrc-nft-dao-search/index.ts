...
function makeQuery(
  ...
  const query: any = { // Query constructor, builds request with params from snapshot space settings
    accountNFTSlots: {
      __args: {
        where: {
          nft_: {
  ...
  if (minter && minter !== '') { //Check to ensure minter id is specified and not blank
    query.accountNFTSlots.__args.where.nft_.minter = minter;
  }

  if (tokens && tokens.length > 0) { //Check to ensure at least 1 token to search for is specified
    query.accountNFTSlots.__args.where.nft_.token_in = tokens;
  }

  if (snapshot !== 'latest') { // If the snapshot date is manually specified, overwrite the 'latest' block, strict inequality check operand used
...
export async function strategy( // *****Logical execution begins here; args passed in by Snapshot settings*****
  ...
  if (!blacklisted_account_ids || blacklisted_account_ids.length === 0) { // If no blacklisted accts specified, set to empty
    blacklisted_account_ids = [''];
  }

  if (!blacklisted_nft_ids || blacklisted_nft_ids.length === 0) { // If no unique nft_ids specified, set to empty
    blacklisted_nft_ids = [''];
  }

  if (!nft_ids || nft_ids.length === 0) { // If no unique nft_ids specified, set to empty
    nft_ids = [''];
  }



  do { // Transmit query and await results
    const response = await subgraphRequest( // Constructs response variable from subgraph query function
      options.graph, // Parameter 1, options specified
      makeQuery( // Query constructor(defined above) called, results are the second parameter
        snapshot,
    ...
    response.accountNFTSlots.forEach((slot) => { // Checking against each accountNFTSlot element
      ...
      balances[slot.account.address] += parseInt(slot.balance); // Otherwise, a bigint is returned, parse it and store in balances array
    });
