...
export async function strategy(
  ...
  const resultLocked: Record<string, [BigNumberish, BigNumberish]> = await multi.execute();


  return Object.fromEntries(
    Object.entries(resultUnlocked).map(([address, balance]) => [
      address,
      parseFloat(formatUnits(BigNumber.from(balance).add(BigNumber.from(resultLocked[address][0])), options.decimals))
    ])
  );
}
